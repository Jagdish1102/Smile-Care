...
public class MedicineDAO {
	...
	public static List<String> getMedicineNames() {
		...
		String sql = "SELECT COALESCE(NULLIF(TRIM(trade_name), ''), " + "NULLIF(TRIM(medicine_name), '')) AS med_name "
				+ "FROM medicines " + "WHERE COALESCE(NULLIF(TRIM(trade_name), ''), "
				+ "NULLIF(TRIM(medicine_name), '')) IS NOT NULL " + "ORDER BY med_name";
	...
	public static List<TemplateSummary> getTemplateSummaries() {
		...
		try (Connection con = DBConnection.connect();
			...
			while (rs.next()) {
				templates.add(new TemplateSummary(rs.getInt("id"), rs.getString("template_name")));
			}
	...
	public static List<MedicineRecord> loadPrescriptionTemplate(int templateId) {
		...
		String sql = "SELECT " + "td.form, " + "td.drug_name, " + "td.instruction, " + "td.quantity, " + "COALESCE("
				+ "NULLIF(TRIM(td.total), ''), " + "NULLIF(TRIM(m.content), ''), " + "CASE "
				+ "WHEN m.weight IS NOT NULL AND TRIM(m.weight) <> '' "
				+ "THEN TRIM(m.weight) || ' ' || COALESCE(TRIM(m.unit), '') " + "ELSE '' " + "END" + ") AS content "
				+ "FROM template_details td "
				+ "LEFT JOIN medicines m ON LOWER(TRIM(m.trade_name)) = LOWER(TRIM(td.drug_name)) "
				+ "WHERE td.template_id = ? " + "ORDER BY td.id";
		...
		try (Connection con = DBConnection.connect(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				...
				while (rs.next()) {
					...
					items.add(new MedicineRecord(form, drugName, content.trim(), instruction, quantity));

					// Debug
					System.out.println("TEMPLATE DEBUG -> " + drugName + " | " + content + " | Qty: " + quantity);
				}
			}
