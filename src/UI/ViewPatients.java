...
public class ViewPatients extends JFrame {
	...
	private JPanel createTopPanel() {
		...
		searchField.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(BORDER_COLOR),
	...
	private JPanel createTablePanel() {
		...
		// ── Model ────────────────────────────────────────────────────
		model = new DefaultTableModel(new String[] { "✓", "ID", "Full Name", "Age", "Gender", "Phone 1", "Phone 2",
				"Disease / Email", "Reg. Date" }, 0) {
			@Override
		...
		// FIX ① – clicking anywhere on a row selects it (smooth, no sharp click
		// required)
	...
	private void applyFilter() {
		...
		if ("Today's Patients".equals(f))
			loadTodayPatients();
		else if ("This Week".equals(f))
			loadThisWeekPatients();
		else if ("This Month".equals(f))
	...
	private void deleteSelected() {
		...
		int w1 = JOptionPane.showConfirmDialog(this,
				...
				"⚠  Step 1 — Confirm Deletion", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
		if (w1 != JOptionPane.YES_OPTION)
			return;
		...
		int w2 = JOptionPane.showConfirmDialog(this,
				...
				"⚠  Step 2 — Final Warning", JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE);
		if (w2 != JOptionPane.YES_OPTION)
			return;
		...
		int passOpt = JOptionPane.showConfirmDialog(this,
				new Object[] { "Enter Admin Password to authorize deletion:", pwField }, "🔒  Admin Password Required",
	...
	private void showDeleteByDateRangeDialog() {
		...
		bWeek.addActionListener(e -> {
			...
			LocalDate s = t.minusDays(t.getDayOfWeek().getValue() - 1);
			fromField.setText(s.toString());
			toField.setText(t.toString());
		});
	...
	private void deletePatientsByDateRange(String from, String to) {
		try {
			...
			if (toDelete.isEmpty()) {
				showStatusMessage("No patients found in that date range", WARNING_COLOR);
				return;
			}

			int count = toDelete.size();
			boolean mass = count > 10;

			// ── Warning 1 ─────────────────────────────
			int w1 = JOptionPane.showConfirmDialog(this,
					"You are about to delete " + count + " patient(s).\n\nDo you want to continue?",
					"⚠ Step 1 — Confirm Deletion", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
			if (w1 != JOptionPane.YES_OPTION)
				return;
			...
			// ── Warning 2 ─────────────────────────────
			int w2 = JOptionPane.showConfirmDialog(this,
					"⚠ FINAL WARNING!\n\n" + "This will permanently delete " + count + " patient(s).\n"
							+ "This action CANNOT be undone.\n\n" + "Are you absolutely sure?",
					"⚠ Step 2 — Final Warning", JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE);
			if (w2 != JOptionPane.YES_OPTION)
				return;
			...
			int passOpt = JOptionPane.showConfirmDialog(this, new Object[] { "Enter Admin Password:", pwField },
					"🔒 Admin Password Required", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
			...
			// ── Extra safety for mass delete ──────────
			if (mass) {
				String typed = JOptionPane.showInputDialog(this,
						"Type DELETE to confirm deletion of " + count + " patients:", "Security Check",
			...
			new SwingWorker<Boolean, Void>() {
				...
				protected Boolean doInBackground() {
					...
					for (Patient p : toDelete) {
						if (PatientDAO.deletePatient(p.getId()))
							ok++;
					}
					return ok > 0;
				}
					...
					try {
						if (get()) {
							...
							JOptionPane.showMessageDialog(ViewPatients.this,
									"Deleted " + count + " patient(s) successfully.", "Done",
	...
	private void editSelected() {
		...
		if (rows.length == 0) {
			showStatusMessage("Select a patient to edit", WARNING_COLOR);
			return;
		}

		if (rows.length > 1) {
			showStatusMessage("Select only one patient to edit", WARNING_COLOR);
			return;
		}
		...
		try {
			int id = Integer.parseInt(safe(model.getValueAt(row, 1)));

			JPanel ep = new JPanel(new GridLayout(6, 2, 10, 10));
			ep.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));

			// ✅ SAFE FETCH (NO NULL CRASH)
			JTextField nameF = new JTextField(safe(model.getValueAt(row, 2)));
			JTextField ageF = new JTextField(safe(model.getValueAt(row, 3)));

			JComboBox<String> genderBox = new JComboBox<>(new String[] { "Male", "Female", "Other" });
			genderBox.setSelectedItem(safe(model.getValueAt(row, 4)));

			JTextField phoneF = new JTextField(safe(model.getValueAt(row, 5)));
			JTextField phone2F = new JTextField(model.getColumnCount() > 6 ? safe(model.getValueAt(row, 6)) : "");

			JTextField diseaseF = new JTextField(model.getColumnCount() > 7 ? safe(model.getValueAt(row, 7)) : "");
			...
			phoneF.setFont(f16);
			phone2F.setFont(f16);
			diseaseF.setFont(f16);

			ep.add(new JLabel("Name:"));
			ep.add(nameF);
			ep.add(new JLabel("Age:"));
			ep.add(ageF);
			ep.add(new JLabel("Gender:"));
			ep.add(genderBox);
			ep.add(new JLabel("Phone 1:"));
			ep.add(phoneF);
			ep.add(new JLabel("Phone 2:"));
			ep.add(phone2F);
			ep.add(new JLabel("Disease:"));
			ep.add(diseaseF);

			int result = JOptionPane.showConfirmDialog(this, ep, "Edit Patient", JOptionPane.OK_CANCEL_OPTION,
					JOptionPane.PLAIN_MESSAGE);

			if (result == JOptionPane.OK_OPTION) {

				int age = 0;
				if (!ageF.getText().trim().isEmpty()) {
					age = Integer.parseInt(ageF.getText().trim());
				}

				Patient p = new Patient(id, nameF.getText().trim(), age, (String) genderBox.getSelectedItem(),
						phoneF.getText().trim(), phone2F.getText().trim(), "", diseaseF.getText().trim(),
						safe(model.getValueAt(row, model.getColumnCount() - 1)));

				if (PatientDAO.updatePatient(p)) {
					loadPatients();
					showStatusMessage("Patient updated successfully", SUCCESS_COLOR);
				} else {
					showStatusMessage("Update failed", DANGER_COLOR);
				}
			}

		} catch (Exception ex) {
			ex.printStackTrace();
			showStatusMessage("Error: " + ex.getMessage(), DANGER_COLOR);
		}
	} // ══════════════════════════════════════════════════════════════════
		// PRESCRIPTION
		// ══════════════════════════════════════════════════════════════════
	...
	private void generatePrescription() {
		int[] rows = getSelectedRows();
		if (rows.length == 0) {
			showStatusMessage("Select a patient for prescription", WARNING_COLOR);
			return;
		}
		if (rows.length > 1) {
			showStatusMessage("Select only one patient for prescription", WARNING_COLOR);
			return;
		}
	...
	private void showSortDialog() {
		...
		String[] options = { "Sort by Age", "Sort by Date", "Sort by Name", "ID Range", "Age Range", "Date Range" };

		int choice = JOptionPane.showOptionDialog(this, "Select sorting / filter option", "Sort Patients",
	...
	private void showDateRangeDialog() {
		...
		int result = JOptionPane.showConfirmDialog(this, panel, "Filter by Date Range", JOptionPane.OK_CANCEL_OPTION);
	...
	private void showIdRangeDialog() {
		...
		int result = JOptionPane.showConfirmDialog(this, panel, "Filter by ID Range", JOptionPane.OK_CANCEL_OPTION,
				JOptionPane.PLAIN_MESSAGE);
	...
	private void showAgeRangeDialog() {
		...
		int result = JOptionPane.showConfirmDialog(this, panel, "Filter by Age Range", JOptionPane.OK_CANCEL_OPTION,
				JOptionPane.PLAIN_MESSAGE);
	...
	private void showExportDateRangeDialog() {
		...
		if (JOptionPane.showConfirmDialog(this, panel, "Export by Date Range", JOptionPane.OK_CANCEL_OPTION,
	...
	private void exportAllPatients() {
		try {
			List<Patient> list = PatientDAO.getAllPatients();
			if (list.isEmpty()) {
				showStatusMessage("No patients to export", WARNING_COLOR);
				return;
			}
			exportToExcel(list,
					"All_Patients_" + LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd")) + ".xlsx",
					"All Patients");
		} catch (Exception e) {
	...
	private void exportTodayPatients() {
		try {
			List<Patient> list = PatientDAO.getTodayPatients();
			if (list.isEmpty()) {
				showStatusMessage("No patients for today", WARNING_COLOR);
				return;
			}
			exportToExcel(list,
					"Today_Patients_" + LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd")) + ".xlsx",
					"Today's Patients");
		} catch (Exception e) {
			showStatusMessage("Export failed: " + e.getMessage(), DANGER_COLOR);
		}
	...
	private void exportPatientsByDateRange(String from, String to) {
		try {
			List<Patient> list = PatientDAO.getPatientsByDateRange(from, to);
			if (list.isEmpty()) {
				showStatusMessage("No patients in that date range", WARNING_COLOR);
				return;
			}
	...
	private void exportSelectedPatients() {
		...
		if (rows.length == 0) {
			showStatusMessage("Select patients to export", WARNING_COLOR);
			return;
		}
		...
		for (int r : rows) {
			list.add(new Patient(Integer.parseInt(model.getValueAt(r, 1).toString()), // ID
					model.getValueAt(r, 2).toString(), // Name
					Integer.parseInt(model.getValueAt(r, 3).toString()), // Age
					model.getValueAt(r, 4).toString(), // Gender
					model.getValueAt(r, 5).toString(), // Phone1
					model.getValueAt(r, 6).toString(), // Phone2 ✅
					"", // Address (if not used)
					model.getValueAt(r, 7).toString(), // Disease
					model.getValueAt(r, 8).toString() // Date
			));
		}
	...
	private void exportToExcel(List<Patient> patients, String fileName, String sheetName) {
		try {
			...
			try (Workbook wb = new XSSFWorkbook(); FileOutputStream fos = new FileOutputStream(file)) {
				...
				String[] cols = { "Patient ID", "Full Name", "Age", "Gender", "Phone 1", "Phone 2", "Disease / Email",
						"Reg. Date" };
				...
				row.createCell(0).setCellValue(PatientIdUtil.format(p.getId()));
					...
					row.getCell(1).setCellStyle(ds);

					row.createCell(2).setCellValue(p.getAge());
					row.getCell(2).setCellStyle(ds);

					row.createCell(3).setCellValue(p.getGender());
					row.getCell(3).setCellStyle(ds);

					row.createCell(4).setCellValue(p.getPhone()); // Phone 1
					row.getCell(4).setCellStyle(ds);

					row.createCell(5).setCellValue(p.getPhone2()); // ✅ Phone 2 (FIX)
					row.getCell(5).setCellStyle(ds);

					row.createCell(6).setCellValue(p.getDisease()); // Disease
					row.getCell(6).setCellStyle(ds);

					row.createCell(7).setCellValue(p.getDate()); // Date
					row.getCell(7).setCellStyle(ds);
				}
				for (int i = 0; i < cols.length; i++)
			...
			JOptionPane.showMessageDialog(this,
					"Export successful!\nFile: " + file.getName() + "\nRecords: " + patients.size(), "Export Complete",
					JOptionPane.INFORMATION_MESSAGE);
	...
	private void showDatePicker(JTextField target) {
		...
		if (JOptionPane.showConfirmDialog(this, p, "Select Date", JOptionPane.OK_CANCEL_OPTION,
				JOptionPane.PLAIN_MESSAGE) == JOptionPane.OK_OPTION) {
			try {
